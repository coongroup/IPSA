...
if (empty($data->sequence)) {
	...
	$peptide = new Peptide($data->sequence, abs($data->precursorCharge), abs($data->charge), $data->fragmentTypes, $data->tolerance, $data->toleranceType, $data->matchingType, $data->cutoff, $basePeak);
	if (empty($data->mods)) {
		$data->mods = array();
	} 
