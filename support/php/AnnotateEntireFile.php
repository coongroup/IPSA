...
class Peptide {
	...
	function MatchFragments() {
		...
		foreach ($this->fragments as $fragment) {
			$fragmentStartTime = explode(' ', microtime());
			...
			foreach ($this->peaks as $peak) {
				if ($tolerance->Contains($peak)) {
					...
					if ($this->matchType == "Intensity" && $peak->intensity >= $this->cutoff) {
						$newMatchedFeature = new MatchedFeature($fragment, $peak->mz, $this->isPPM, FormatProcessingTime($fragmentStartTime));
		...
		for ($i = $this->precursorCharge; $i > 0; $i--) {
			...
			foreach ($this->peaks as $peak) {
				if ($tolerance->Contains($peak)) {
					$newMatchedFeature = new MatchedFeature($precursor, $peak->mz, $this->isPPM, FormatProcessingTime($fragmentStartTime));
					array_push($peak->matchedFeatures, $newMatchedFeature);
				}
			}
		}
...
class MatchedFeature {
	...
	function MatchedFeature($matchedFeature, $matchedPeakMz, $isPPM, $time) {
...
function FormatProcessingTime($startTime) {
	...
	$decimal = floatval($endTime[0]) - floatval($startTime[0]);
	$seconds = floatval($endTime[1]) - floatval($startTime[1]);
...
if (!$data->fragmentTypes->C->selected && !$data->fragmentTypes->a->selected && !$data->fragmentTypes->b->selected && !$data->fragmentTypes->c->selected && !$data->fragmentTypes->Z->selected && !$data->fragmentTypes->x->selected && !$data->fragmentTypes->y->selected && !$data->fragmentTypes->z->selected) {
	...
	fputcsv($summaryOutput, array('Scan Number','Sequence','Theoretical Mz','Charge','Modifications <Name;Index;Mass Change>','# Matched Fragments','# Bonds Broken','% TIC Explained','Processing Time','Peak Count'));
	fputcsv($fragmentOutput, array('Scan Number','Fragment Type','Fragment Bond Number','Neutral Loss','Fragment Charge','Intensity','Experimental Mz','Theoretical Mz','Mass Error (ppm)','% Base Peak','% TIC', 'Processing Time'));
	
	for ($i = 0; $i < count($IDs); $i++) {
		$peptideStartTime = explode(' ', microtime());
		$id = $IDs[$i];
		Feature::$tic = 0;
		...
		fputcsv($summaryOutput, array($id->scanNumber, $peptide->sequence, $peptide->precursorMz, $peptide->precursorCharge, FormatPeptideModsForDownload($peptide->modifications), count($matchedFeatureList), CalculateBondsBroken($matchedFeatureList, strlen($peptide->sequence)) , $percentTic . "%", FormatProcessingTime($peptideStartTime), count($peptide->peaks)));
		
		for($j = 0; $j < count($matchedFeatureList); $j++) {
			$feature = $matchedFeatureList[$j]->feature;
			$peak = $experimentalFeatureList[$j];
			fputcsv($fragmentOutput, array($id->scanNumber, $feature->type, $feature->number, " " . $feature->neutralLoss, $feature->charge, $peak->intensity, $peak->mz, $feature->mz, $matchedFeatureList[$j]->massError, $peak->percentBasePeak . "%", $peak->percentTic . "%", $matchedFeatureList[$j]->processingTime));
		}
	}
	// close files
	...
	echo (json_encode(Array("downloadPath" => $relativeZipPath . $timeStamp . "_Results.zip", "fileName" => $timeStamp . "_Results.zip")));
}
?>